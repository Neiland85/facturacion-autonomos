...
describe('InvoiceService', () => {
    let invoiceService;
    beforeEach(() => {
        invoiceService = new Usuario_1.InvoiceService();
    });
...
async function name() {
                expect(xml).toContain('<fe:Facturae>'); // Ajusta la etiqueta raíz del XML
}
            expect(xml).toContain('<InvoiceNumber>FAC-001</InvoiceNumber>');
            expect(xml).toContain('<IssueDate>2025-06-03</IssueDate>'); // Verifica la fecha de emisión
            expect(xml).toContain('<Description>Servicio de consultoría</Description>'); // Verifica el concepto
        });
        test('Debe lanzar un error al intentar parsear un XML inválido', async () => {
            ...
            await expect(invoiceService.parseXmlInvoice(invalidXml)).rejects.toThrow('Failed to parse XML invoice');
        });
    });
